...
Solution Planner::solve()
{
    ...
    while (!OPEN.empty() && !is_expired(deadline)) {
        ...
        refiner_pool.remove_if([&](auto &proc) {
            if ((proc).wait_for(TIME_ZERO) != std::future_status::ready)
                return false;
            apply_new_solution(proc.get());
            ++seed_refiner;
            refiner_pool.emplace_back(std::async(std::launch::async,
                                                 &Planner::get_refined_plan,
                                                 this, backtrack(H_goal)));
            return true;
        });
        ...
        if (H_goal == nullptr && is_same_config(H->C, ins->goals)) {
            ...
            info(1, verbose, deadline,
                 "found initial solution, cost: ", H_goal->g);
    ...
    if (min_f_val < INT_MAX) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = i + 1; j < N; j++) {
                ...
                if (i_group != j_group) {
                    if (in_field_of_view(Q_to[i], Q_to[j],
...
void Planner::set_refiner()
{
    ...
    for (auto k = 0; k < REFINER_NUM; ++k) {
        ++seed_refiner;
        refiner_pool.emplace_back(std::async(
            std::launch::async, &Planner::get_refined_plan, this, plan));
    }
}
