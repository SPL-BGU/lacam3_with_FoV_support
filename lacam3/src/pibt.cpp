                    ...
                    Vertex *node, int agent)
{
    ...
    for (const Vertex *current_node : field_of_view) {
        if (occupied_field_of_view[current_node->id].is_occupied_by_agent(
                agent)) {
                           ...
                           const std::vector<int> &order)
{
    ...
    for (auto i = 0; i < N; ++i) {
        ...
        if (can_occupy(occupied_now, occupied_field_of_view_now, Q_from[i],
            ...
            throw std::runtime_error("Failed in initialization - the node (" +
                                     std::to_string(Q_from[i]->x) + "," +
                                     std::to_string(Q_from[i]->y) +
        ...
        for (auto i : order) {
            if (Q_to[i] == nullptr) {
                ...
                if (affected_agents.empty()) {
                    if (first_agent) {  // if the first agent fails, no need to
                                        // continue - the pibt must allow the
                                        // highest priority agent to move.
                        success = false;
                        break;
                    }
                    ...
                    if (nullptr != Q_to[i]) {
                        throw std::runtime_error(
                            ...
                            " has a next location, but the affected agents are "
                            "empty.");
                    }
                    if (!can_occupy(occupied_next, occupied_field_of_view_next,
                                    Q_from[i], i)) {
                        std::cout << "Agent " << i
                                  << " cannot occupy its current location ("
                                  << Q_from[i]->x << "," << Q_from[i]->y << ")"
                                  << std::endl;
                        std::cout << "The following agents are at the "
                                     "following locations:"
                                  << std::endl;
                        for (auto j = 0; j < N; j++) {
                            std::cout << "Agent " << j << " is at ("
                                      << Q_from[j]->x << "," << Q_from[j]->y
                                      << ")" << std::endl;
                        }
                        throw std::runtime_error(
                            "The agent " + std::to_string(i) +
                    ...
                    // Failed to plan the next location for agent i, keep it in
                    // it's current location
                    occupy(occupied_next, occupied_field_of_view_next,
                           Q_from[i], i);
                    Q_to[i] = Q_from[i];
                }
            }
                            ...
                            Vertex *v_pusher_origin, Vertex *v_puller_origin)
{
    ...
    while (D->get(pusher, v_puller) < D->get(pusher, v_pusher)) {
        ...
        for (auto u : v_puller->neighbor) {
            ...
            if (u == v_pusher || (u->neighbor.size() == 1 && i != NO_AGENT &&
                                  ins->goals[i] == u)) {
                --n;
            } else {
                tmp = u;
